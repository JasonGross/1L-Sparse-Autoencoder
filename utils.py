...
os.environ["TRANSFORMERS_CACHE"] = str((WORKSPACE_DIR / "cache/").absolute())
os.environ["DATASETS_CACHE"] = str((WORKSPACE_DIR / "cache/").absolute())
Path(os.environ["TRANSFORMERS_CACHE"]).mkdir(exist_ok=True, parents=True)
Path(os.environ["DATASETS_CACHE"]).mkdir(exist_ok=True, parents=True)
# %%
...
# %%
SAVE_DIR = Path(WORKSPACE_DIR / "1L-Sparse-Autoencoder/checkpoints")
...
class AutoEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        normalization = cfg.get("normalization", "l1")
    ...
    def forward(self, x):
        ...
        match self.normalization:
            ...
            case "log":
                l1_loss = acts.float().abs().log1p().sum()
            case "prod":
                l1_loss = acts.float().abs().log1p().sum().exp()
        l1_loss = self.l1_coeff * l1_loss
...
if loading_data_first_time:
    ...
    data.save_to_disk(WORKSPACE_DIR / "data/c4_code_tokenized_2b.hf")
    ...
    torch.save(
        all_tokens_reshaped, WORKSPACE_DIR / "data/c4_code_2b_tokens_reshaped.pt"
    )
else:
    # data = datasets.load_from_disk(WORKSPACE_DIR / "data/c4_code_tokenized_2b.hf")
    all_tokens = torch.load(WORKSPACE_DIR / "data/c4_code_2b_tokens_reshaped.pt")
